...
class AliquotProfile:
    ...
    collection_model = django_apps.get_model(app_config.app_label, 'collection')
    panel_model = django_apps.get_model(app_config.app_label, 'panel')
    aliquot_model = django_apps.get_model(app_config.app_label, 'aliquot')
    aliquot_type_model = django_apps.get_model(app_config.app_label, 'aliquottype')
    profile_model = django_apps.get_model(app_config.app_label, 'aliquotprofile')
    profile_item_model = django_apps.get_model(app_config.app_label, 'aliquotprofileitem')
    ...
    def receive(self, requisition, subject_identifier=None):
        ...
        if requisition.is_drawn == YES:
            try:
                receive = self.collection_model.objects.get(
