...
class Result(BaseResult, BaseUuidModel):
    ...
    def save(self, *args, **kwargs):
        ...
        if not self.review:
            self.review = Review.objects.create(
                title='{0} for {1}'.format(
                    self.result_identifier, self.order.aliquot.receive.registered_subject.subject_identifier))
    ...
    def to_order(self):
        return '<a href="/admin/lab_clinic_api/order/?q={order_identifier}">order</a>'.format(
    ...
    def to_items(self):
        if self.release_status.lower() == 'new':
            ...
            return '<a href="/admin/lab_clinic_api/resultitem/?q={result_identifier}">items</a>'.format(
